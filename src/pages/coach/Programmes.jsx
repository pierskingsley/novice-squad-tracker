  ...
  return (
    <div className="px-4 pt-6">
      <div className="flex items-center justify-between mb-5">
        <h1 className="text-2xl font-bold text-slate-900 dark:text-slate-50">Programmes</h1>
        <button
        ...
        <div className="space-y-3">
          {programmes.map(prog => {
            ...
            const upcoming = prog.programme_assignments
              ...
              ?.slice(0, 2)
            return (
              <div key={prog.id} className="bg-white dark:bg-[#1C1C1E] rounded-2xl border border-slate-200 dark:border-zinc-800 overflow-hidden shadow-sm">
                <button onClick={() => navigate(`/coach/programmes/${prog.id}/edit`)} className="w-full px-4 py-4 flex items-start justify-between text-left">
                  <div className="flex-1 min-w-0">
                    <div className="text-sm font-bold text-slate-900 dark:text-slate-50 truncate">{prog.name}</div>
                    <div className="flex items-center gap-3 mt-1.5">
                      <span className="text-xs text-slate-400 dark:text-slate-500">{exerciseCount} exercise{exerciseCount !== 1 ? 's' : ''}</span>
                      <span className="text-slate-300 dark:text-zinc-600 text-xs">•</span>
                      <span className="text-xs text-slate-400 dark:text-slate-500">{assignmentCount} assignment{assignmentCount !== 1 ? 's' : ''}</span>
                    </div>
                  ...
                  <ChevronRight size={16} className="text-slate-400 dark:text-zinc-500 mt-0.5 flex-shrink-0" />
                </button>
                <div className="border-t border-slate-100 dark:border-zinc-800 flex">
                  <button onClick={() => navigate(`/coach/programmes/${prog.id}/edit`)}
                    className="flex-1 flex items-center justify-center gap-1.5 py-2.5 text-xs text-slate-400 dark:text-zinc-500 hover:text-slate-700 dark:hover:text-slate-200 hover:bg-slate-50 dark:hover:bg-zinc-800 transition-colors">
                    <Edit2 size={13} /> Edit
                  </button>
                  <div className="w-px bg-slate-100 dark:bg-zinc-800" />
                  <button onClick={() => setConfirmDup(prog)} disabled={duplicating === prog.id}
                    className="flex-1 flex items-center justify-center gap-1.5 py-2.5 text-xs text-slate-400 dark:text-zinc-500 hover:text-slate-700 dark:hover:text-slate-200 hover:bg-slate-50 dark:hover:bg-zinc-800 transition-colors disabled:opacity-40">
      ...
      <Modal open={!!confirmDup} onClose={() => setConfirmDup(null)} title="Duplicate programme?">
        <p className="text-slate-500 dark:text-slate-400 text-sm mb-5">
          Creates a copy of <strong className="text-slate-900 dark:text-slate-100">"{confirmDup?.name}"</strong> without assignments.
        </p>
        <div className="flex gap-3">
          <button onClick={() => setConfirmDup(null)} className="flex-1 py-3 rounded-xl text-sm font-medium bg-slate-100 dark:bg-zinc-800 text-slate-600 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-zinc-700 transition-colors">Cancel</button>
