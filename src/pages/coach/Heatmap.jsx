...
export default function Heatmap() {
  ...
  return (
    <div className="px-4 pt-6 pb-10">
      <h1 className="text-2xl font-bold text-slate-900 dark:text-slate-50 mb-1">Squad Heatmap</h1>
      <p className="text-xs text-slate-400 dark:text-slate-500 mb-5">Last {DAYS} days of training activity</p>
        ...
        <div className="bg-white dark:bg-[#1C1C1E] rounded-2xl border border-slate-200 dark:border-zinc-800 overflow-hidden shadow-sm">
          <div className="overflow-x-auto">
            <table className="w-full text-xs">
              <thead>
                <tr>
                  <th className="text-left px-3 py-2 text-slate-400 dark:text-slate-500 font-medium whitespace-nowrap w-24">Athlete</th>
                  {days.map(d => {
                    const date = new Date(d + 'T00:00:00')
                    const isMonday = date.getDay() === 1
                    const dayNum = date.getDate()
                    return (
                      <th key={d} className={`px-0.5 py-2 text-center font-normal ${isMonday ? 'text-slate-500 dark:text-slate-400' : 'text-slate-300 dark:text-zinc-600'}`}>
                        {isMonday ? dayNum : ''}
                      </th>
                    )
                  })}
              ...
              <tbody className="divide-y divide-slate-50 dark:divide-zinc-800">
                {athletes.map(athlete => {
                  ...
                  return (
                    <tr key={athlete.id} className="hover:bg-slate-50 dark:hover:bg-zinc-800 transition-colors">
                      <td className="px-3 py-2 font-medium text-slate-900 dark:text-slate-100 whitespace-nowrap">
                        <div>{athlete.name.split(' ')[0]}</div>
                        <div className="text-slate-400 dark:text-slate-500 font-normal">{count} session{count !== 1 ? 's' : ''}</div>
                      </td>
                      {days.map(d => {
                          ...
                          <td key={d} className="px-0.5 py-2">
                            <div className={`w-5 h-5 rounded-sm mx-auto transition-colors ${
                              didTrain
                                ? 'bg-vesta-red'
                                : isToday
                                ? 'bg-slate-200 dark:bg-zinc-700 ring-1 ring-slate-300 dark:ring-zinc-500'
          ...
          <div className="flex items-center gap-3 px-3 py-2.5 border-t border-slate-100 dark:border-zinc-800">
            <span className="text-xs text-slate-400 dark:text-slate-500">Legend:</span>
            <div className="flex items-center gap-1.5">
              <div className="w-4 h-4 rounded-sm bg-vesta-red" />
              <span className="text-xs text-slate-500 dark:text-slate-400">Trained</span>
            </div>
            <div className="flex items-center gap-1.5">
              <div className="w-4 h-4 rounded-sm bg-slate-100 dark:bg-zinc-800" />
