  ...
  async function load() {
    ...
    try {
      ...
      const { data: existingSets } = await supabase
        .from('sets').select('*').in('session_exercise_id', seIds)
  ...
  async function addExercise() {
    ...
    try {
      ...
      const { data: newSE, error: ie } = await supabase
        .from('session_exercises')
        .insert({ session_id: id, exercise_id: selectedExerciseId, order_index: orderIndex, notes: '' })
        .select('*, exercises(id, name)')
        .single()
      if (ie) throw ie
    ...
    try {
      ...
      if (exData) {
        ...
        const { data: pb } = await supabase
          .from('personal_bests').select('weight')
          .eq('athlete_id', user.id).eq('exercise_id', exerciseId).maybeSingle()

        if (!pb || weight > pb.weight) {
          await supabase.from('personal_bests').upsert({
            athlete_id: user.id, exercise_id: exerciseId, weight, reps,
            achieved_at: new Date().toISOString(), set_id: setRow.id,
  ...
  if (error) {
    return (
      <div className="px-4 pt-16 text-center">
        ...
        <button onClick={() => navigate(-1)} className="mt-4 text-amber-500 text-sm">Go back</button>
      </div>
    )
  }
  ...
  return (
    <div className="px-4 pt-6 pb-28">
      ...
      <div className="flex items-center gap-3 mb-5">
        ...
        <div>
          <p className="text-xs text-zinc-500 uppercase tracking-wider font-medium">{sessionDate}</p>
          <h1 className="text-xl font-bold text-white leading-tight">Edit session</h1>
        </div>
      </div>
